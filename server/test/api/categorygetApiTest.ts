...
const getUrl = "/api/manage/categories/inventory";
const postUrl = "/api/manage/categories";
...
describe("Categories API - Get categories", async () => {
	...
	test("after adding 2 categories, 2 are returned with correct properties", async () => {
		...
		const addResponse1 = await api.post(postUrl).send(addRequest1);
		const addResponse2 = await api.post(postUrl).send(addRequest2);

		assert.strictEqual(addResponse1.statusCode, 201);
		assert.strictEqual(addResponse2.statusCode, 201);

		const getCategoriesResponse = await api.get(getUrl);
