...
describe('Items API - Delete Item', () => {
  test('item can be deleted using its ID', async () => {
    ...
    const categoriesResponse = await api.get(categoriesGetUrl);
    ...
    const afterDeleteResponse = await api.get(categoriesGetUrl);
...
describe('Items API - Update Item', () => {
  test('item can be updated using its ID', async () => {
    ...
    const verifyResponse = await api.get(categoriesGetUrl);
    ...
    assert.strictEqual(updatedItem.item_data.name, 'Updated Horse');
    assert.strictEqual(updatedItem.item_data.age, '10');
    assert.strictEqual(updatedItem.item_data.wins, '42');
  });
